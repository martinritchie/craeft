function [A, Sd] = Connect_repeated(Sd, sg)
% ------------------------------------------------------------------------
...
% current selection is disregarded and a new one is made. This is repeated
% until both conditions are satisfied. This process has the advantage of
% very fast running times but will bias the sampling procedure. Please see
% the above link for more details. Written by Martin Ritchie, University of
% Sussex, 2016.
% ------------------------------------------------------------------------
...
%% Connection process
repeats = 0;
while attempt < max_attempt && reset == 0
    for i = 1:M
        attempt = 0;
        c = sum(sg{i});
        ...
        while ~isempty(Sg_bin) && sum(cellfun(@isempty,Sg_bin(1:positions(i))))==0
            ...
            while ( mean(sum(A([nodes{:}],[nodes{:}])))>0 || ... % multiedge
