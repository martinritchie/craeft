function [A, Sd, refuse] = Connect_refuse(Sd, sg)
% ------------------------------------------------------------------------
...
% tuple of nodes is selected the algorithm will check for: (1) duplicate
% selections of nodes and (2) connections that already exist between nodes
% in the current selection. If either of these conditions are met the
% process is aborted and the connection procedure will need to be
% restarted. This process has the advantage of unbiased sampling but at
% considerable computational cost that is highly sensitive to the mean
% dregree. Please see the above link for more details. Written by Martin
% Ritchie, University of Sussex, 2016.
% ------------------------------------------------------------------------
...
for i = 1:M
    for j = 1:positions(i)
        ...
        Sg_bin{index1} = Sg_bin{index1}(randperm(length(Sg_bin{index1})));
        index1 = index1 + 1;
    end
end
...
for i = 1:M
    ...
    c = sum(sg{i});
    % Generate the correct proportions of corners.
    if ic_subgraph(i) == 1;
    ...
    while ~isempty(Sg_bin) && sum(cellfun(@isempty,Sg_bin(1:positions(i))))==0
        ...
        for j = 1:positions(i)
            ...
            while length(unique(nodei{j}))~=porportions(j)
                nodei{j} = randi(length(Sg_bin{j}),1,porportions(j));
            end
            attempt = attempt + 1;
            nodes{j} = Sg_bin{j}(nodei{j});
        end
        if ( mean(sum(A([nodes{:}],[nodes{:}])))>0 || ... % multiedge
                length(unique([nodes{:}]))<length(sg{i}) ) % selfedge
            refuse = refuse + 1;
            return
        end
        % Form the subgraph
